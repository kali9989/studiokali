import { useState, useEffect, useRef } from 'react';

export default function VHSOverlay() {
    const timeRef = useRef<HTMLSpanElement>(null);
    const [showTracking, setShowTracking] = useState(false);

    useEffect(() => {
        const updateTime = () => {
            if (!timeRef.current) return;
            const now = new Date();
            const h = String(now.getHours()).padStart(2, '0');
            const m = String(now.getMinutes()).padStart(2, '0');
            const s = String(now.getSeconds()).padStart(2, '0');
            timeRef.current.textContent = `${h}:${m}:${s}`;
        };
        updateTime();
        const interval = setInterval(updateTime, 1000);
        return () => clearInterval(interval);
    }, []);

    // Random VHS tracking glitch — less frequent
    useEffect(() => {
        let timeout: ReturnType<typeof setTimeout>;
        const scheduleNext = () => {
            timeout = setTimeout(() => {
                setShowTracking(true);
                setTimeout(() => setShowTracking(false), 250);
                scheduleNext();
            }, 8000 + Math.random() * 12000);
        };
        scheduleNext();
        return () => clearTimeout(timeout);
    }, []);

    return (
        <div className="fixed inset-0 pointer-events-none z-9000" aria-hidden="true">
            {/* Scanlines — GPU-composited */}
            <div className="vhs-scanlines" />

            {/* Noise Grain — reduced opacity and size */}
            <div className="vhs-noise" />

            {/* Tracking Glitch Band */}
            {showTracking && <div className="vhs-tracking-band" />}

            {/* VHS Timestamp — no re-renders, ref-based */}
            <div className="fixed top-6 left-6 z-9001 flex items-center gap-3 pointer-events-none select-none">
                <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                    <span className="font-jetbrains text-[10px] text-red-500/80 uppercase tracking-[0.3em] font-bold">REC</span>
                </div>
                <span ref={timeRef} className="font-jetbrains text-[11px] text-white/40 tracking-widest tabular-nums">
                    00:00:00
                </span>
            </div>

            {/* Bottom VHS badge */}
            <div className="fixed bottom-6 left-6 z-9001 pointer-events-none select-none">
                <span className="font-jetbrains text-[9px] text-white/20 tracking-[0.4em] uppercase">
                    PLAY ▶ SP
                </span>
            </div>
        </div>
    );
}
